...
type DeviceConfig struct {
	...
	AWGParameters      map[string]string
}
...
func ParseInterface(cfg *ini.File, device *DeviceConfig) error {
	...
	device.AWGParameters = make(map[string]string)
	awgKeys := []string{
		"Jc", "Jmin", "Jmax",
		"S1", "S2", "S3", "S4",
		"H1", "H2", "H3", "H4",
	...
	for _, name := range awgKeys {
		if key, keyErr := section.GetKey(name); keyErr == nil && strings.TrimSpace(key.String()) != "" {
			device.AWGParameters[awgIPCKey(name)] = strings.TrimSpace(key.String())
		}
	}

	if key, keyErr := section.GetKey("HeaderProtectionKey"); keyErr == nil && strings.TrimSpace(key.String()) != "" {
		hexKey, decodeErr := encodeBase64ToHex(strings.TrimSpace(key.String()))
		if decodeErr != nil {
			return errors.New("invalid HeaderProtectionKey: " + decodeErr.Error())
		}
		device.AWGParameters["header_protection_key"] = hexKey
	}

	return nil
}

func awgIPCKey(name string) string {
	switch name {
	case "ContentPaddingAddition":
		return "content_padding_addition"
	case "RekeyAfterTime":
		return "rekey_after_time"
	case "RekeyTimeout":
		return "rekey_timeout"
	case "RejectAfterTime":
		return "reject_after_time"
	case "KeepaliveTimeout":
		return "keepalive_timeout"
	case "MaxHandshakeAttempts":
		return "max_handshake_attempts"
	default:
...
func ParsePeers(cfg *ini.File, peers *[]PeerConfig) error {
	...
	for _, section := range sections {
		...
		if sectionKey, err := section.GetKey("PersistentKeepalive"); err == nil {
			peer.KeepAlive = strings.TrimSpace(sectionKey.String())
		}
