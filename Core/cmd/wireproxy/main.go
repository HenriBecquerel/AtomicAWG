...
func main() {
	...
	parser := argparse.NewParser("awgproxy", "Userspace WireGuard and AmneziaWG client with SOCKS5/HTTP proxy")
	...
	if *printVerison {
		fmt.Printf("awgproxy, version %s (AmneziaWG v3 engine)\n", version)
		return
	}
	...
	for _, spawner := range conf.Routines {
		go func(spawner wireproxy.RoutineSpawner) {
			if err := spawner.SpawnRoutine(tun); err != nil {
				log.Fatal(err)
			}
		}(spawner)
	}
