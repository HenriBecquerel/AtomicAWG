...
// SetLogOutput redirects both the standard logger and the error logger of this
// package, so an embedding application (e.g. a GUI) can capture core logs.
func SetLogOutput(w io.Writer) {
	log.SetOutput(w)
	errorLogger.SetOutput(w)
}

// lifecycle tracks the listeners spawned for a VirtualTun so the tunnel can be
// shut down cleanly by an embedding application.
...
// RegisterCloser records a listener owned by this tunnel. If the tunnel is
// already stopped the closer is closed immediately.
func (d *VirtualTun) RegisterCloser(c io.Closer) {
	if d.life == nil {
		return
	}
	d.life.mu.Lock()
	stopped := d.life.stopped
	if !stopped {
		d.life.closers = append(d.life.closers, c)
	}
	d.life.mu.Unlock()
	if stopped {
		_ = c.Close()
...
// Stop closes every registered listener and shuts down the WireGuard device.
// It is safe to call multiple times.
func (d *VirtualTun) Stop() {
	if d.life != nil {
		d.life.mu.Lock()
		alreadyStopped := d.life.stopped
		d.life.stopped = true
		closers := d.life.closers
		d.life.closers = nil
		d.life.mu.Unlock()
		if alreadyStopped {
			return
...
// It returns once the routine has finished serving (or failed to start).
type RoutineSpawner interface {
	SpawnRoutine(vt *VirtualTun) error
}
...
func (config *Socks5Config) SpawnRoutine(vt *VirtualTun) error {
	...
	if err != nil {
		return fmt.Errorf("socks5 listen on %s failed: %w", config.BindAddress, err)
	}
	vt.RegisterCloser(listener)

	if err := server.Serve(listener); err != nil && !errors.Is(err, net.ErrClosed) {
		return fmt.Errorf("socks5 serve failed: %w", err)
	}
	return nil
}

// SpawnRoutine spawns a http server.
func (config *HTTPConfig) SpawnRoutine(vt *VirtualTun) error {
	server := &HTTPServer{
		config: config,
		dial:   vt.Tnet.Dial,
	...
	listener, err := server.listen("tcp", config.BindAddress)
	if err != nil {
		return fmt.Errorf("http proxy listen on %s failed: %w", config.BindAddress, err)
	}
	vt.RegisterCloser(listener)
...
// serveAccept runs an accept loop, tolerating transient errors (e.g. EMFILE)
// so a single failed accept does not bring down the whole process.
func serveAccept(name string, server net.Listener, handle func(net.Conn)) {
	...
	for {
		...
		if err != nil {
			...
			// Some listener implementations (e.g. netstack) do not wrap
			// net.ErrClosed, so bail out after persistent failures.
			consecutiveFailures++
			if consecutiveFailures >= 50 {
				errorLogger.Printf("%s: giving up after repeated accept failures: %v\n", name, err)
				return
			}
			errorLogger.Printf("%s: accept failed: %v\n", name, err)
...
func (conf *TCPClientTunnelConfig) SpawnRoutine(vt *VirtualTun) error {
	...
	vt.RegisterCloser(server)

	serveAccept("TCPClientTunnel", server, func(conn net.Conn) {
		tcpClientForward(vt, raddr, conn)
	})
	return nil
}

// SpawnRoutine connects to the specified target and plumbs it to STDIN / STDOUT
func (conf *STDIOTunnelConfig) SpawnRoutine(vt *VirtualTun) error {
	raddr, err := parseAddressPort(conf.Target)
	if err != nil {
		return err
	}
...
func (conf *TCPServerTunnelConfig) SpawnRoutine(vt *VirtualTun) error {
	raddr, err := parseAddressPort(conf.Target)
	if err != nil {
		return err
	}
	...
	serveAccept("TCPServerTunnel", server, func(conn net.Conn) {
		tcpServerForward(vt, raddr, conn)
	})
	return nil
}

// SpawnRoutine spawns an SNI proxy server.
func (config *SNIConfig) SpawnRoutine(vt *VirtualTun) error {
	listener, err := net.Listen("tcp", config.BindAddress)
	if err != nil {
		return err
	}
	...
	serveAccept("SNIProxy", listener, func(conn net.Conn) {
		sniServe(vt.Tnet.Dial, conn)
	})
	return nil
}
